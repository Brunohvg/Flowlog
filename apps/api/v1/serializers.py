    ...
    def validate(self, data):
        if not data.get("customer_id") and not data.get("customer_name"):
            raise serializers.ValidationError(
                {"customer_name": "Informe customer_id ou customer_name"}
            )
        if not data.get("customer_id") and not data.get("customer_phone"):
            raise serializers.ValidationError(
                {"customer_phone": "Informe customer_id ou customer_phone"}
            )
        return data
...
class PaymentLinkSerializer(serializers.ModelSerializer):
    ...
    order_code = serializers.CharField(
        source="order.code", read_only=True, allow_null=True
    )
    ...
    def validate(self, data):
        if not data.get("order_id"):
            if not data.get("amount"):
                raise serializers.ValidationError(
                    {"amount": "Informe order_id ou amount"}
                )
            if not data.get("customer_name"):
                raise serializers.ValidationError(
                    {"customer_name": "Informe order_id ou customer_name"}
                )
        return data
