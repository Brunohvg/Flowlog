...
class DashboardView(APIView):
    ...
    def get(self, request):
        ...
        paid_filter = orders.filter(payment_status=PaymentStatus.PAID).exclude(
            order_status=OrderStatus.CANCELLED
        )

        revenue_today = paid_filter.filter(sale_date=today).aggregate(
            t=Sum("total_value")
        )["t"] or Decimal("0")

        revenue_month = paid_filter.filter(sale_date__gte=month_start).aggregate(
            t=Sum("total_value")
        )["t"] or Decimal("0")
