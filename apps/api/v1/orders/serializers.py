...
class OrderCreateSerializer(serializers.Serializer):
    ...
    delivery_type = serializers.ChoiceField(
        choices=["pickup", "motoboy", "sedex", "pac"], 
        default="motoboy"
    )
    ...
    def validate(self, data):
        if not data.get("customer_id"):
            if not data.get("customer_name"):
                raise serializers.ValidationError({"customer_name": "Obrigatório sem customer_id"})
            if not data.get("customer_phone"):
                raise serializers.ValidationError({"customer_phone": "Obrigatório sem customer_id"})
        return data
