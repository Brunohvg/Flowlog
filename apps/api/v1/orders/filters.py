...
class OrderFilter(filters.FilterSet):
    ...
    status = filters.ChoiceFilter(
        field_name="order_status", choices=OrderStatus.choices, label="Status do Pedido"
    )
    ...
    date_from = filters.DateFilter(
        field_name="sale_date", lookup_expr="gte", label="Data inicial"
    )

    date_to = filters.DateFilter(
        field_name="sale_date", lookup_expr="lte", label="Data final"
    )

    customer = filters.UUIDFilter(field_name="customer_id", label="ID do Cliente")

    search = filters.CharFilter(
        method="filter_search", label="Busca (código ou cliente)"
    )

    min_value = filters.NumberFilter(
        field_name="total_value", lookup_expr="gte", label="Valor mínimo"
    )

    max_value = filters.NumberFilter(
        field_name="total_value", lookup_expr="lte", label="Valor máximo"
    )
    ...
    def filter_search(self, queryset, name, value):
        ...
        return queryset.filter(
            models.Q(code__icontains=value) | models.Q(customer__name__icontains=value)
        )
