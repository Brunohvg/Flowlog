...
from apps.integrations.whatsapp.client import EvolutionAPIError, EvolutionClient
...
def _get_instance_client(tenant_settings):
    ...
    if (
        not api_url
        or not tenant_settings.evolution_instance
        or not tenant_settings.evolution_instance_token
    ):
        return None
...
def whatsapp_setup(request):
    ...
    if not api_configured:
        context["error"] = (
            "Evolution API não configurada no sistema. Contate o administrador."
        )
            ...
            try:
                ...
                if status.get("connected") != tenant_settings.whatsapp_connected:
                    ...
                    tenant_settings.save(
                        update_fields=["whatsapp_connected", "whatsapp_number"]
                    )
...
def whatsapp_create_instance(request):
    ...
    if not _is_api_configured():
        return JsonResponse(
            {"success": False, "error": "Evolution API não configurada no sistema"},
            status=400,
        )
    ...
    if not instance_name:
        return JsonResponse(
            {"success": False, "error": "Informe um nome para a instância"}, status=400
        )
    ...
    existing = (
        TenantSettings.objects.filter(evolution_instance=instance_name)
...
def whatsapp_get_qrcode(request):
    ...
    if not tenant_settings or not tenant_settings.evolution_instance:
        return JsonResponse(
            {"success": False, "error": "Instância não configurada"}, status=400
        )
    ...
    if not tenant_settings.evolution_instance_token:
        try:
            ...
            if global_client:
                ...
                result = global_client.ensure_instance(
                    tenant_settings.evolution_instance
                )
    ...
    if not client:
        return JsonResponse(
            {"success": False, "error": "Token da instância não configurado"},
            status=400,
        )
    ...
    try:
        ...
        if (
            "já está conectada" in error_msg.lower()
            or "already connected" in error_msg.lower()
        ):
            return JsonResponse(
                {
                    "success": True,
...
def whatsapp_check_status(request):
    ...
    try:
        ...
        return JsonResponse({"configured": True, "connected": False, "error": str(e)})
...
def whatsapp_test_message(request):
    ...
    if not tenant_settings or not tenant_settings.is_whatsapp_configured:
        return JsonResponse(
            {"success": False, "error": "WhatsApp não configurado"}, status=400
        )

    phone = request.POST.get("phone", "").strip()
    if not phone:
        return JsonResponse(
            {"success": False, "error": "Informe o número de telefone"}, status=400
        )
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": f"Mensagem enviada para {phone}"}
        )
