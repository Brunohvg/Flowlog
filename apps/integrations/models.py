...
class NotificationLog(models.Model):
    ...
    tenant = models.ForeignKey(
        "tenants.Tenant", on_delete=models.CASCADE, related_name="notification_logs"
    )
    ...
    recipient_phone = models.CharField(
        max_length=20, help_text="Últimos 4 dígitos do telefone"
    )
    recipient_name = models.CharField(max_length=200, blank=True)

    # Conteúdo
    message_preview = models.TextField(
        max_length=500, blank=True, help_text="Preview da mensagem (truncado)"
    )

    # Resposta da API
    api_response = models.JSONField(
        null=True, blank=True, help_text="Resposta completa da Evolution API"
    )
...
class APIRequestLog(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    correlation_id = models.CharField(
        max_length=50, db_index=True, help_text="ID de correlação para rastreamento"
    )
