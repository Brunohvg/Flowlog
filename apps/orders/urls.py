...
urlpatterns = [
    ...
    path(
        "<uuid:order_id>/entregar/", order_mark_delivered, name="order_mark_delivered"
    ),
    path(
        "<uuid:order_id>/falha-entrega/",
        order_mark_failed_attempt,
        name="order_mark_failed_attempt",
    ),
    # Fluxo de retirada
    path(
        "<uuid:order_id>/liberar-retirada/",
        order_ready_for_pickup,
        name="order_ready_for_pickup",
    ),
    path(
        "<uuid:order_id>/retirado/", order_mark_picked_up, name="order_mark_picked_up"
    ),
    ...
    # Alterações
    path(
        "<uuid:order_id>/alterar-entrega/",
        order_change_delivery,
        name="order_change_delivery",
    ),
    path("<uuid:order_id>/duplicar/", order_duplicate, name="order_duplicate"),
    path(
        "<uuid:order_id>/reenviar-notificacao/",
        order_resend_notification,
        name="order_resend_notification",
    ),
    # Impressão
