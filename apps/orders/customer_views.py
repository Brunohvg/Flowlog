...
def customer_list(request):
    ...
    repeat_customers = (
        all_customers.annotate(order_count=Count("orders"))
    ...
    avg_ticket = (
        (orders_agg["total"] or 0) / orders_agg["count"] if orders_agg["count"] else 0
    )
...
def customer_detail(request, customer_id):
    ...
    customer = get_object_or_404(
        Customer.objects.for_tenant(request.tenant), id=customer_id
    )
...
def customer_edit(request, customer_id):
    ...
    if request.method == "POST":
        ...
        existing = (
            ...
            .filter(phone_normalized=phone_normalized)
        ...
        if existing:
            messages.error(
                request, f"Já existe um cliente com este telefone: {existing.name}"
            )
            return redirect("customer_edit", customer_id=customer_id)
